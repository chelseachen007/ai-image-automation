...
export class EngineConfigManager {
   ...
   * 获取引擎的模型列表
   */
  static getEngineModels(type: AIEngineType, modelType?: 'text' | 'image' | 'video' | 'multimodal'): AIEngineModel[] {
  ...
  static validateEngineConfig(config: AIEngineConfig): boolean {
    return !!(config.id && config.name && config.type && config.baseUrl && config.apiKeyName);
  }
   ...
   * 获取引擎的默认模型
   */
  static getDefaultModel(type: AIEngineType, modelType: 'text' | 'image' | 'video' | 'multimodal' = 'text'): AIEngineModel | null {
